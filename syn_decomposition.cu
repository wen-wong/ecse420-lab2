...
__global__ void synthesis_interior_elements(float *u, float *u1, float *u2, int siz, int num_of_elements) {
    ...
    for (int op = 0; op < op_per_thread; op++) {
        int index = ((blockIdx.x * blockDim.x) + threadId.x) * op_per_thread + op;
        if (index >= size * size) {
            return;
        }
...
void synthesis(float *u, float *u1, float *u2, int size, int num_of_iterations, float *result, int num_of_elements, int num_of_blocks, int num_of_threads) {
    ...
    for (int i = 0; i < num_of_iterations; i++) {
        ...
        synthesis_boundary_elements<<<num_of_blocks, num_of_threads>>>(u, size);
        cudaDeviceSynchronize();
        synthesis_corner_elements<<<num_of_blocks, num_of_threads>>>(u, size);
        cudaDeviceSynchronize();
